    ...
    async function fetchAndDisplayJSON() {
      try {
        ...
        // Puzzle: hhow to use just one statement to include current Year in the first year column 
            ...
            for (const [key, value] of Object.entries(items)) {
              value.forEach(item => {
                ...
                for (let i = 0; i < 30; i++) {
                  ...
                  const remainingLife = replacementYear - year
                  const firstFutureCost = baseCost * ((1 + inflationRate) ** (replacementYear - year))

  
                //  const isReplacementYear = (year + usefulLife - replacementYear) % usefulLife === 0 || (year + 1 - replacementYear) % usefulLife === 0;
                //  // console.log(isReplacementYear);
                //  const understandMathPow =  firstFutureCost * Math.pow((1 + inflationRate), (year - replacementYear));
                //   // console.log(understandMathPow);
                //  const understandModulo = firstFutureCost * (1 + inflationRate) ** (year - replacementYear);
                //   console.log(understandModulo);
                  ...
                  if ((year - replacementYear) % usefulLife === 0) {
                    ...
                    fullyFunded = (baseCost * Math.pow((1 + inflationRate), (year - currentYear)) * ((year + usefulLife - replacementYear) % usefulLife)) / usefulLife;
                  }
                  columnSums[i] += fullyFunded;
                  console.log(baseCost);
                  console.log(baseCost * Math.pow((1 + inflationRate), (year - currentYear)));
    ...
    if (lastRow) {
      ...
      } catch (error) { console.error('Error fetching or displaying JSON data:', error); }
    }


    // Call the function to fetch and display JSON data fetchAndDisplayJSON(); 
    fetchAndDisplayJSON();
